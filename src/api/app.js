...
app.get('/', (req, res) => {
  ...
  }).then(res => res.json()).then(json => res.json(transformer(json.resultSets[0]))).catch(json => res.json(json));
});

app.get('/leaguedashplayerstats', (req, res) => {
  fetch('http://stats.nba.com/stats/leaguedashplayerstats?College=&Conference=&Country=&DateFrom=&DateTo=&Division=&DraftPick=&DraftYear=&GameScope=&GameSegment=&Height=&LastNGames=0&LeagueID=00&Location=&MeasureType=Base&Month=0&OpponentTeamID=0&Outcome=&PORound=0&PaceAdjust=N&PerMode=PerGame&Period=0&PlayerExperience=&PlayerPosition=&PlusMinus=N&Rank=N&Season=2016-17&SeasonSegment=&SeasonType=Regular+Season&ShotClockRange=&StarterBench=&TeamID=0&VsConference=&VsDivision=&Weight=', {
    method: 'GET',
    headers: {
      'Referer': 'http://stats.nba.com/',
      'DNT': '1',
